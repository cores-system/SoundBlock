...
namespace CSCore.CoreAudioAPI
{
    ...
    public class AudioSessionEnumerator : ComObject, IEnumerable<AudioSessionControl>
    {
        ...
        public AudioSessionEnumerator(IntPtr ptr) : base(ptr)
        {
            if (IsComError)
                IsError = true;
        }

        public AudioSessionEnumerator()
        {
            IsError = true;
        }
        ...
        public int Count
        {
            get
            {
                if (IsError || IsComError)
                    return 0;

                int count;
                var res = GetCountNative(out count);
                if (res == -1)
                    return 0;
        ...
        public AudioSessionControl GetSession(int index)
        {
            ...
            var res = GetSessionNative(index, out session);

            if (index < 0 || res == -1 || session == null || session.IsComError || session.IsError)
                return new AudioSessionControl();
